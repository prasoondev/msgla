"""
eval_perplexity_pg19_gla.py — Perplexity and forgetting-curve evaluation for GLA-family checkpoints.
...
Examples:
    # Evaluate a single DCP checkpoint (auto-detects model type)
...
def maybe_plot(
    ...
    ax.set_title(f"PG19 Perplexity vs. Position Bucket — {model_type} (up to 32 K)", fontsize=13)
...
def main():
    parser = argparse.ArgumentParser(
        description="Perplexity + forgetting-curve evaluation on PG19 for GLA-family checkpoints"
    )
    # ── checkpoint args ──────────────────────────────────────────
    parser.add_argument(
        "--checkpoint_path",
    ...
    # ── Summary table ────────────────────────────────────────────
    print("\n" + "=" * 76)
    print(f"  RESULTS SUMMARY  —  PG19 Perplexity  ({detected_model_type})")
    print("=" * 76)
    print(
        f"  {'Label':<20} {'Type':<8} {'Corpus PPL':>11} "
        f"{'Mean Doc PPL':>13} {'BPB':>8} {'Tokens':>10}"
    )
    print("-" * 76)
    ...
    # ── Forgetting-curve inline preview ──────────────────────────
    if curves_by_label:
        print("\n  FORGETTING CURVES  (mean NLL per position bucket, first 10 buckets)")
    ...
    if args.plot_output and curves_by_label:
        maybe_plot(curves_by_label, args.plot_output, detected_model_type)
