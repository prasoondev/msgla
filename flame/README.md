...
This repository also includes a built-in custom model named `ms_gla` under `custom_models/ms_gla`.
`MS-GLA` extends GLA with multiple temporal branches, where each branch runs a smaller GLA module at its own scale and the outputs are fused back together token-wise.

Current implementation highlights:

* The total attention head budget is preserved across scales through `scale_num_heads`, so multiscale capacity does not come from simply increasing total heads.
* The first scale must be `1`, which guarantees a fine-resolution branch is always present.
* Training, cache-based decoding, and `generate()` are supported.
* Tensor parallel support is implemented for `model_type="ms_gla"`.
* The currently supported pooling mode is `avg`.
* Supported fusion modes are `softmax` and `mean`.

Useful files:

* Model config: `custom_models/ms_gla/config_msgla.py`
* Layer implementation: `custom_models/ms_gla/ms_gla_layer.py`
* Model wrapper: `custom_models/ms_gla/modeling_msgla.py`
* Example configs:
...
* Correctness tests: `tests/test_msgla.py`
* TP plan tests: `tests/test_msgla_tp.py`
* Scale-config load test: `tests/test_msgla_scale_configs.py`
* Single-variant benchmark: `tests/benchmark_msgla.py`
* Scale sweep benchmark: `tests/compare_msgla_scales.py`

Recommended starting points:

* Use `configs/ms_gla_340M_s12.json` for the first efficiency-oriented training run.
* Use `configs/ms_gla_340M.json` for the first research-oriented comparison against a richer multiscale decomposition.

For implementation notes, cache behavior, and experiment guidance, see `custom_models/ms_gla/README.md`.
