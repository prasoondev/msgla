"""
eval_perplexity_pg19.py — Perplexity-vs-position-bucket evaluation for MS-GLA checkpoints.
...
Examples:
    # Evaluate a single DCP checkpoint on PG19 + SlimPajama test
...
def maybe_plot(
    ...
    ax.set_title(
        f"{dataset_name} Perplexity vs. Position Bucket (up to 32 K)",
        fontsize=13,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Perplexity-vs-position-bucket evaluation on HF text datasets for MS-GLA checkpoints"
    )
    # ── checkpoint args ──────────────────────────────────────────
    parser.add_argument(
        "--checkpoint_path",
    ...
    # ── Summary table ────────────────────────────────────────────
    print("\n" + "=" * 72)
    print("  RESULTS SUMMARY  —  Perplexity vs Position Bucket")
    print("=" * 72)
    for dataset_name in dataset_names:
        ...
        print("  POSITION BUCKET CURVES  (mean perplexity per position bucket)")
    ...
    if args.results_json and all_results:
        # position_bucket_perplexity keys are ints → convert to str for JSON
        serialisable = []
    ...
    if args.plot_output and curves_by_dataset:
        ...
        for dataset_name in dataset_names:
            ...
            maybe_plot(curves_by_label, output_path, dataset_name)
